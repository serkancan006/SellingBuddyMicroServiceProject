    ...
    public class EventBusRabbitMQ : BaseEventBus
    {
        ...
        public EventBusRabbitMQ(EventBusConfig config, IServiceProvider serviceProvider) : base(config, serviceProvider)
        {
            if (config.Connection != null)
            {
                ...
                {
                    // Self referencing loop detected for propery
            ...
            consumerChannel = createdConsumerChannel();
        ...
        private void SubsManager_OnEventRemoved(object? sender, string eventName)
        {
            ...
            //using var channel = persistentConnection.CreateModel(); // sil
        ...
        public override void Subscribe<T, TH>()
        {
            ...
            if (!SubsManager.HasSubscriptionsForEvent(eventName))
            {
                ...
                consumerChannel.QueueDeclare(queue: GetSubName(eventName), // Enusre queue exists while consumng
                                            durable: true,
                                            exclusive: false,
                                            autoDelete: false,
                                            arguments: null);
        ...
        private IModel createdConsumerChannel()
        {
            if(!persistentConnection.IsConnected)
            {
                persistentConnection.TryConnect();
